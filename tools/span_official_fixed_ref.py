...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--activated-skip", action="store_true", help="Use activated c1 output for the SPAB skip returned to conv_cat.")
...
class SpanFixedRef:
    def __init__(
        ...
        self.shift = shift
        self.activated_skip = activated_skip
    ...
    def spab(self, x: list[list[list[int]]], block: int) -> tuple[list[list[list[int]]], list[list[list[int]]]]:
        ...
        return out, out1_act if self.activated_skip else out1
...
def main() -> None:
    ...
    ref = SpanFixedRef(
        ...
        info["channels"],
        args.scale_shift,
        activated_skip=args.activated_skip,
    )
    out = ref.forward(img)
