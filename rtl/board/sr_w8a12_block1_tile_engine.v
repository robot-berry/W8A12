...
// Tile-local W8A12 block_1 compute engine.
//
// This is the first real compute primitive intended to replace the dummy block
// acknowledgement in the 6-pass tile scheduler. It consumes one feature tile
// after start, streams it through the existing REDS W8A12 block_1 SPAB frontend,
// and pulses done after one output tile is produced.
module sr_w8a12_block1_tile_engine #(
    ...
    wire block_s_ready;
    wire block_s_valid = active && !input_done && s_valid;
    wire input_take = block_s_valid && block_s_ready;
    ...
    assign ready = !active;
    assign busy = active;
    assign s_ready = active && !input_done && block_s_ready;
    ...
    span_w8a12_block1_streamed_frontend #(
        ...
        .clk(clk),
        .rst(rst),
        .s_valid(block_s_valid),
        .s_ready(block_s_ready),
        .s_feat(s_feat),
        .s_user(s_user),
        .s_last(s_last),
        .m_valid(m_valid),
