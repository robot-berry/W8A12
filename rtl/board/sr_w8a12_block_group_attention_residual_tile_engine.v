...
// Tile-local attention/residual postprocess with runtime block-selected tables.
module sr_w8a12_block_group_attention_residual_tile_engine #(
    ...
    wire side_user_mismatch = (s_c3_user != s_res_user);
    wire side_last_mismatch = (s_c3_last != s_res_last);
    wire [8:0] input_count_dbg;
    wire [8:0] output_count_dbg;
    ...
    generate
        if (PIX_W >= 9) begin : gen_debug_count_wide
            assign input_count_dbg = input_count[8:0];
            assign output_count_dbg = output_count[8:0];
        end else begin : gen_debug_count_narrow
            assign input_count_dbg = {{(9-PIX_W){1'b0}}, input_count};
            assign output_count_dbg = {{(9-PIX_W){1'b0}}, output_count};
        end
    endgenerate
    ...
    (* keep_hierarchy = "yes", dont_touch = "yes" *)
    span_w8a12_block_group_attention #(
        .ACT_W(ACT_W),
        .BLOCKS(BLOCKS),
        .BLOCK_W(BLOCK_W),
        .OUT3_REQUANT_Q31_FILE(OUT3_REQUANT_Q31_FILE),
        .RESIDUAL_REQUANT_Q31_FILE(RESIDUAL_REQUANT_Q31_FILE),
