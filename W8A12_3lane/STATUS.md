...
| x2 720p4.48 降级闭合档 | packed 2-D scheduler-level xsim | PASS：900-DSP 门限内 `24x72` 为 4.483fps @250MHz、888 DSP；该项是当前 x2 资源门限内性能闭合点，不替代原 x2 720p20 目标 | `evidence/sim_fps_design_space/packed2d_x2_direct_xsim_replay/summary.md` |
...
| `jtag_true2x2_dbg3_single_boundary_20260703` | RTL/BUILD PASS / BOARD FAIL CLEAN LOCALIZED | dbg3/single-boundary 行为级 RTL raw compare PASS：`0/192` mismatch；bitstream timing PASS。实板 probe/PSU/register read PASS，输出完整 `192/192`、`frame_done=1`、`error=0`，compare FAIL `192/192`、PSNR `7.724 dB`。`src_feat0_hash=0xF7F21881` 与 RTL 匹配，`src_b1_hash=0xF657C1B4` 与 RTL 期望 `0x16ede581` 不一致 | 当前连接、读写链路和 conv1 feat0 source 边界可用；下一步拆 SPAB block1 输出/feature buffer replay 到 `src_b1` 的边界 |
| `jtag_true2x2_dbg4_bank4_20260703` | RTL/BUILD PASS / BOARD FAIL LOCALIZED STALL | dbg4/bank4 行为级 RTL raw compare PASS：`0/192` mismatch；bitstream timing PASS，WNS `12.079ns`、WHS `0.009ns`，LUT `39698`、REG `116621`、BRAM `311`、DSP `128`。实板 probe/PSU/register read PASS，但 smoke 输出 `0/192`，`counter_in=4`、`counter_out=0`、`frame_done=0`、`error=0`，delayed read 不变。`bank4_sched_feat0_hash=0xF7F21CC2` 与 RTL 匹配，`bank4_sched_b1_hash=0x811C9DC5` 与 RTL 期望 `0x16EDE1C2` 不一致 | 首个可见异常进一步压到 SPAB block1 输出/attention 到 scheduler block1 完成握手附近；因本轮 stall，下一步改用更窄的单一 valid/count 探针，避免深调试 bank 影响完成路径 |
| `jtag_true2x2_dbg5_countview_20260703` | RTL/BUILD PASS / BOARD FAIL BLOCK1 C1 STALL | dbg5/count-view 行为级 RTL raw compare PASS：`0/192` mismatch；bitstream timing PASS，WNS `12.580ns`、WHS `0.010ns`，LUT `39799`、REG `116685`、BRAM `311`、DSP `128`。实板 probe/PSU/register read PASS，但 smoke 输出 `0/192`，delayed read 不变。bank5 期望 `C1/C2/C3/ATT=0x00030003`、`block_counts=0x00060018`、`replay=0x18`；板端为 `C1=0x00000003`、`C2/C3/ATT=0`、`block_counts=0x00010000`、`replay=0x4` | mismatch/stall 进一步定位为 SPAB block1 C1 后没有推进到 C2/C3/attention/输出完成；下一步只导出 C1->C2 ready/valid 或 feature replay valid/ready 单项 |
| `jtag_true2x2_dbg6_c1c2_detail_20260703` | RTL PASS / BUILD PARTIAL / BOARD NEXT | endpoint 已将现有 C1/C2 detail 信号接到 debug bank6；true2x2 行为级 raw compare PASS：`0/192` mismatch、max diff 0、PSNR Infinity。bank6 RTL 期望：C1 detail/core/lane/io=`0x0000200c/0x17c01af5/0x0dbf1af5/0x00021403`，C2 detail/core/lane=`0x0000200c/0x17c01af5/0x0dbf1af5`。bitstream attempt 生成 synth/opt checkpoint 和综合资源快照：LUT 42817、REG 115888、BRAM 311、DSP 126，但未生成 `.bit` | 下一步从 `jwtw_wrapper_opt.dcp` 继续 place/route/write_bitstream，或重跑 full bitstream；上板后读 bank6，对比 C1->C2 detail，判断板端卡在 C1 output、C2 input、core/window/replay ready 的哪一侧 |
...
| 项目 | 状态 | 证据 | 备注 |
| --- | --- | --- | --- |
| Windows/USB 级探测 | READY IN LATEST DBG5 RUN | `board_runs/jtag_w8a12_tile_writer/true2x2_dbg5_countview_20260703/probe/usb_jtag_devices.json`；历史 `W8A12_3lane/evidence/board_probe/jtag_precondition_current/summary.md` | dbg5 acceptance 前置探测显示 USB/JTAG target 可见；旧 current summary 仅保留为历史断点 |
| Vivado JTAG target 探测 | READY IN LATEST DBG5 RUN | `board_runs/jtag_w8a12_tile_writer/true2x2_dbg5_countview_20260703/probe/probe_vivado_hw_targets.log`；`board_runs/jtag_w8a12_tile_writer/true2x2_dbg5_countview_20260703/probe_step.log` | dbg5 acceptance 中 Vivado probe exit=0，随后 PSU init、smoke、register read 均能访问板端；当前问题已不是 target 不可见 |
...
| W8A12 board recovery preflight | RECOVERED IN LATEST DBG5 RUN | `board_runs/jtag_w8a12_tile_writer/true2x2_dbg5_countview_20260703/stagehash_true2x2_acceptance_summary.md` | 当前板卡链路已能进入 stage-hash/debug-bank 上板；剩余问题是 W8A12 block1/datapath 运行时 stall/mismatch |
