...
| 项目 | 状态 | 证据 | 备注 |
| --- | --- | --- | --- |
| Windows/USB 级探测 | CURRENT BLOCKED / HISTORICAL PASS | `W8A12_3lane/evidence/board_probe/recovery_preflight_current/usb/usb_jtag_devices.txt`；`board_runs/vivado_hw_target_probe_after_replug_20260628` | 当前 USB match=3、known JTAG candidate=0；历史重插记录曾出现 known candidate 和 Vivado target |
| Vivado JTAG target 历史探测 | HISTORICAL PASS | `W8A12_3lane/evidence/board_probe/vivado_hw_probe.md` | 重插后曾经 target count=1，device count=2，`xczu19_0` 和 `arm_dap_1` 可见；当前状态以后续 precondition/probe 行为准 |
...
| JTAG precondition current | BLOCKED | `W8A12_3lane/evidence/board_probe/jtag_precondition_current/summary.md` | USB known candidate=0、Vivado target count=not_checked；因 USB 侧无在线已知 JTAG 设备，当前按流程未进入 Vivado probe，恢复到 USB known candidate>=1 且 target count>=1 后再运行 stage-hash true2x2 acceptance |
...
5. 当前技术阻塞点已推进到“W8A12 true2x2 板端数值不一致”，但最新物理连接前置条件又回到 JTAG 枚举阻塞：常规 preflight 为 USB known JTAG candidate=0、Vivado target=not_checked；强制 Vivado probe 后为 Vivado target=0。恢复 JTAG 后，需回到 Default/inpixfix baseline 用更窄 stage hash 定位 writer 前偏差。
