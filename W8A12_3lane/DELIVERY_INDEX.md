...
| 最新上板续跑进展 | `evidence/board_reports/jtag_true2x2_dbg5_countview_20260703/analysis.md` | 2026-07-03 dbg5/count-view 实板复跑：RTL raw compare PASS、bitstream/timing PASS，JTAG/PSU/register read PASS；板端输出仍为 `0/192`，但 bank5 将 stall 进一步压到 SPAB block1 C1 后：RTL 期望 `C1/C2/C3/ATT=0x00030003`、`block_counts=0x00060018`、`replay=0x18`，板端为 `C1=0x00000003`、`C2/C3/ATT=0`、`block_counts=0x00010000`、`replay=0x4`。该 run 作为 block1 C1->C2/feature replay 握手定位证据，clean correctness baseline 仍以 dbg3/single-boundary 为准 |
...
当前严格审计为 `72 / 76`，新增赛题报告、PDF/Word 报告导出、PPA 汇总、报告完整性检查、画质指标闭环门禁、stage-hash 上板流程静态检查、JTAG recovery checklist、board validation readiness、submission manifest/archive 和 evidence matrix 均已形成，剩余 4 项均为真实板端 validation。历史 true2x2 stage-hash 上板曾恢复 JTAG/PSU/register read，并完整输出 `192/192` 且 `frame_done=1`、`error=0`，但 compare FAIL：`191/192` mismatch，PSNR `11.8292 dB`，最早失败边界为 `tail_b1_hash`。2026-07-03 full probe 已恢复到 `READY`，USB known JTAG candidate count=3，Vivado target count=1；dbg2/source-b6 上板可完整输出 `192/192`，但因 `error=0x0000000C` 只作为探针侵入性风险证据。随后非侵入 stagehash baseline 复跑得到 `error=0`、`frame_done=1`、输出 `192/192`，但 compare FAIL `192/192`、PSNR `11.884 dB`。dbg3/single-boundary 版本已完成 RTL raw compare `0/192`、bitstream/timing PASS 和真实上板复跑；板端仍 compare FAIL，但保持 `error=0`，且 `src_feat0_hash=0xf7f21881` 与 RTL 匹配，首个已知 mismatch 前移到 `src_b1_hash`。dbg4/bank4 进一步确认 `sched_feat0_hash` 匹配而 `sched_b1_hash` 不匹配并触发 no-output stall。最新 dbg5/count-view 显示 block1 仅有 C1 侧部分计数，C2/C3/attention 计数为 0，`block_counts=0x00010000`、`replay=0x4`，因此当前 clean 结论仍以 dbg3 为准：连接、读写链路和 conv1 feat0 source 边界可用；定位证据已把下一个排查点压到 SPAB block1 C1->C2/feature replay ready-valid 边界。
