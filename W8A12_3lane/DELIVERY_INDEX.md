...
| 最新上板续跑进展 | `evidence/board_reports/jtag_true2x2_dbg2_src_boundary_20260703_goal_continue/analysis.md` | 2026-07-03 dbg2/source-b6 实板续跑：JTAG/PSU/register read PASS，2x2 输出完整 `192/192`，compare FAIL `188/192`，PSNR 19.263 dB，error flags=`0x0000000C`，需回退到低侵入 single-boundary probe |
...
| JTAG-W8A12 dbg2 source-boundary 一键验收 | `scripts/run_w8a12_dbg2_source_boundary_acceptance.ps1`、`evidence/board_reports/jtag_true2x2_dbg2_src_boundary_prepare_20260629.md`、`evidence/board_reports/jtag_true2x2_dbg2_src_boundary_20260703_goal_continue/analysis.md` | 已提供；RTL raw compare PASS、bitstream/timing/resource PASS。2026-07-03 已完成一次真实上板：target READY、PSU init PASS、输出完整，但 compare FAIL 且 `error=0x0000000C`，本轮 hash 视为 debug probe 侵入性证据 |
