...
| JTAG 当前前置条件 | `evidence/board_probe/jtag_precondition_current/summary.md`、`evidence/board_probe/jtag_precondition_usb_only_current/summary.md` | 2026-07-03 完整复测为 READY：USB known JTAG candidate=3，Vivado target count=1，可进入小图上板验收 |
| W8A12 board recovery preflight | `scripts/run_w8a12_board_recovery_preflight.ps1`、`W8A12_3lane/scripts/run_w8a12_board_recovery_preflight.ps1`、`board_runs/w8a12_board_recovery_preflight/goal_continue_20260703_fullprobe/board_recovery_preflight_summary.md` | 已提供；`-SkipVivadoProbe` 可在后台 Vivado 运行时只刷新 USB-only 证据。本轮 full probe PASS，target count=1 |
| 最新上板续跑进展 | `evidence/board_reports/jtag_true2x2_dbg6_c1c2_detail_20260703/analysis.md`、`evidence/implementation_runs/jtag_true2x2_dbg6_build_attempt_20260703/summary.md` | dbg6 已将 C1/C2 detail 信号接到 debug bank6，RTL raw compare 仍 `0/192` mismatch；bank6 期望值已记录。dbg6 build 已生成 synth/opt checkpoint 和综合资源快照，但尚未生成 `.bit`，下一步是继续 place/route/write_bitstream 后上板读取 bank6 |
| dbg5/count-view 定位证据 | `evidence/board_reports/jtag_true2x2_dbg5_countview_20260703/analysis.md` | RTL raw compare PASS、bitstream/timing PASS，JTAG/PSU/register read PASS；板端输出仍为 `0/192`，但 bank5 将 stall 进一步压到 SPAB block1 C1 后：RTL 期望 `C1/C2/C3/ATT=0x00030003`、`block_counts=0x00060018`、`replay=0x18`，板端为 `C1=0x00000003`、`C2/C3/ATT=0`、`block_counts=0x00010000`、`replay=0x4`。该 run 作为 block1 C1->C2/feature replay 握手定位证据，clean correctness baseline 仍以 dbg3/single-boundary 为准 |
| clean stagehash baseline | `evidence/board_reports/jtag_true2x2_stagehash_baseline_rerun_20260703_goal_continue/analysis.md` | 非侵入 stagehash baseline：输出完整 `192/192`、`error=0`，compare FAIL `192/192`，PSNR 11.884 dB；作为 dbg3/single-boundary 的前置 clean baseline |
...
| JTAG 物理恢复清单 | `docs/jtag_recovery_checklist.md`、`tools/generate_jtag_recovery_checklist.py`、`evidence/board_probe/jtag_recovery_checklist/summary.md`、`evidence/board_probe/jtag_recovery_checklist_usb_only_current/summary.md` | 历史 BLOCKED/READY 清单保留；最新 full probe 为 READY，USB known candidate=3 且 Vivado target count=1 |
...
| BRAM 消耗计算 | `tools/calc_bram_accounting.py`、`evidence/resource/bram_accounting/summary.md` | PASS；x4 W8A12/F48 模型常量下限 119.32 BRAM36、32x32 tile buffer 下限 142.23 BRAM36；true2x2/JTAG-W8A12 为 311/545，A5 32x32 attempt 为 415.5/545 |
| bitstream/PPA 门槛 | `evidence/bitstream_ppa_gate/summary.md` | PASS_WITH_SCOPE；true2x2/JTAG-W8A12 bitstream 已生成。最新 dbg5/count-view 实现为 LUT 39799、FF 116685、BRAM 311、DSP 128、WNS 12.580ns、WHS 0.010ns；不声明 720p packed 2-D 完整 bitstream |
...
| JTAG-W8A12 dbg2 source-boundary 一键验收 | `scripts/run_w8a12_dbg2_source_boundary_acceptance.ps1`、`evidence/board_reports/jtag_true2x2_dbg2_src_boundary_prepare_20260629.md`、`evidence/board_reports/jtag_true2x2_dbg2_src_boundary_20260703_goal_continue/analysis.md` | 已提供；RTL raw compare PASS、bitstream/timing/resource PASS。2026-07-03 已完成一次真实上板：target READY、PSU init PASS、输出完整，但 compare FAIL 且 `error=0x0000000C`，本轮 hash 视为 debug probe 侵入性证据；随后已用非侵入 stagehash baseline 复跑得到 `error=0` clean mismatch |
...
当前严格审计为 `72 / 76`，新增赛题报告、PDF/Word 报告导出、PPA 汇总、BRAM 消耗计算、报告完整性检查、画质指标闭环门禁、stage-hash 上板流程静态检查、JTAG recovery checklist、board validation readiness、submission manifest/archive 和 evidence matrix 均已形成，剩余 4 项均为真实板端 validation。历史 true2x2 stage-hash 上板曾恢复 JTAG/PSU/register read，并完整输出 `192/192` 且 `frame_done=1`、`error=0`，但 compare FAIL：`191/192` mismatch，PSNR `11.8292 dB`，最早失败边界为 `tail_b1_hash`。2026-07-03 full probe 已恢复到 `READY`，USB known JTAG candidate count=3，Vivado target count=1；dbg2/source-b6 上板可完整输出 `192/192`，但因 `error=0x0000000C` 只作为探针侵入性风险证据。随后非侵入 stagehash baseline 复跑得到 `error=0`、`frame_done=1`、输出 `192/192`，但 compare FAIL `192/192`、PSNR `11.884 dB`。dbg3/single-boundary 版本已完成 RTL raw compare `0/192`、bitstream/timing PASS 和真实上板复跑；板端仍 compare FAIL，但保持 `error=0`，且 `src_feat0_hash=0xf7f21881` 与 RTL 匹配，首个已知 mismatch 前移到 `src_b1_hash`。dbg4/bank4 进一步确认 `sched_feat0_hash` 匹配而 `sched_b1_hash` 不匹配并触发 no-output stall。dbg5/count-view 显示 block1 仅有 C1 侧部分计数，C2/C3/attention 计数为 0，`block_counts=0x00010000`、`replay=0x4`。最新 dbg6 已接出 C1/C2 detail bank 并保持 RTL bit-exact，但 dbg6 bitstream 尚未生成；因此当前 clean 结论仍以 dbg3 为准，定位证据已把下一个排查点压到 SPAB block1 C1->C2/feature replay ready-valid 边界。
