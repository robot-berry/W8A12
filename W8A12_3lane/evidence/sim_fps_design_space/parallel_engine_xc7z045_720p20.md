...
## Best 250 MHz 720p20 Candidates

| Budget | Output lanes | Tap lanes | MAC lanes | Est. DSP | Cycles/LR px | FPS 320x180 @250MHz | FPS 320x180 @200MHz | Efficiency |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 80% DSP | `none` | `none` | `none` | `none` | `none` | `<20` | `<20` | `n/a` |
| 100% DSP | `none` | `none` | `none` | `none` | `none` | `<20` | `<20` | `n/a` |
...
At `200 MHz`, the full REDS-trained W8A12 SPAN must sustain X4 `320x180 -> 1280x720 @20fps`. At `250 MHz`, the full-model path becomes more plausible, but still depends on high parallelism across both output channels and taps plus sustained INT8 MAC utilization.

This does not replace RTL synthesis. It selects the next frame-engine target: keep W8A12 as the quality/reference model, prototype the smallest 250 MHz candidate that reaches 720p20 in this sizing table, then compare RTL/board output against the integer W8A12 reference preview on every test.
