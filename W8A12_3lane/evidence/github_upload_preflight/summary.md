...
| `git.outside_changes_detected` | PASS | `654 outside status lines; do not stage them for W8A12 upload` |
| `git.no_staged_outside_w8a12` | PASS | `[]` |
| `git.upload_candidate_count` | PASS | `795` |
...
| `submission_manifest.has_files` | PASS | `335` |
...
This preflight permits an INCOMPLETE draft package, but blocks upload if the target remote is not configured or if staged files include paths outside W8A12_3lane/.
