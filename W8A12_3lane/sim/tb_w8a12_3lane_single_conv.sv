...
module tb_w8a12_3lane_single_conv;
    ...
    logic signed [ACT_W-1:0] expected [0:TOTAL-1];
    ...
    w8a12_3lane_conv_layer #(
        .IN_CH(CH),
        .LANES(LANES),
        .OUT_CH_PER_LANE(OUT_CH_PER_LANE),
    ...
    initial begin
        ...
        read_vector(`W8A12_3LANE_A0_EXPECTED_TXT, expected);
        ...
        while (out_pix < PIXELS) begin
            ...
            wait_cyc = wait_cyc + 1;
            if (wait_cyc > 5000000)
                $fatal(1, "timeout waiting for outputs got=%0d expected=%0d", out_pix, PIXELS);
        end

        if (mismatches != 0)
            $fatal(1, "FAIL w8a12_3lane_single_conv mismatches=%0d", mismatches);
        $display("PASS w8a12_3lane_single_conv pixels=%0d channels=%0d", PIXELS, CH);
        $finish;
    end
