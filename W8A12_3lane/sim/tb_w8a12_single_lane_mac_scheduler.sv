...
module tb_w8a12_single_lane_mac_scheduler;
    ...
    initial begin
        ...
        if (mismatches != 0)
            $fatal(1, "FAIL w8a12_single_lane_mac_scheduler mismatches=%0d", mismatches);
        $display("PASS w8a12_single_lane_mac_scheduler pixels=%0d lane_ch=%0d", PIXELS, LANE_CH);
        $finish;
    end

    always @(posedge clk) begin
        if (!rst && m_valid && m_ready) begin
            for (ch = 0; ch < LANE_CH; ch = ch + 1) begin
