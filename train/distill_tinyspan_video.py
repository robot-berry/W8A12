...
def append_csv(path: Path, row: dict) -> None:
    ...
    for attempt in range(5):
        try:
            ...
            if attempt == 4:
                recovery = path.with_name(f"{path.stem}_recovered{path.suffix}")
...
def save_training_artifacts(
    ...
    last_batch: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor] | None,
    checkpoint_name: str,
    preview_name: str,
    epoch: int | None = None,
) -> None:
    ...
    torch.save(ckpt, output / checkpoint_name)
